...
def get_idces_uniform_linear(neighborhood_size):
    ...
    def get_idx(layer):
        ...
        idces_b = idces_w[idces_w[:,1]==n_input][:,0] # renverra forcement rien car construit avec arange -> max = n-1 de n_input
        idces_w = idces_w[idces_w[:,1]<n_input]
        return idces_w, idces_b  # retourne un liste des indices et une liste vide
    return get_idx
...
def get_idces_filter_conv():
    ...
    def get_idx(layer):
        ...
        return idces_w, idx_filter # retourne un seul id
    return get_idx
...
def build_selector(config):
    ...
    selector_class = getattr(current_module, selector_name) # Selector()
...
class Selector():
    ...
    def __init__(self, config):
        ...
        self.layer_distr = torch.cumsum(torch.Tensor(self.layer_distr),0) # pour 0.5,0.5 -> 0.5,1.0
