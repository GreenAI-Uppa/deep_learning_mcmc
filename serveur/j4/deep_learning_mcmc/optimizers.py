...
class MCMCOptimizer(Optimizer):
    def __init__(self, sampler, data_points_max = 1000000000, iter_mcmc=1, lamb=1000,  prior=None, selector=None, pruning_level=0, queue=None):
        """
    ...
    async def train_1_epoch(self, dataloader, model, loss_fn, verbose=False, test_dataloader=None):
        """
        ...
        # write output data columns
        # self.doc.write(f'id_batch;test_acc;test_loss;loss;arl0;arl1\n')
        ...
        for id_batch, (X, y) in enumerate(dataloader):
            ...
            X = X.to(device)
            y = y.to(device)
            acceptance_ratio += await self.train_1_batch(X, y, model, dataloader, loss_fn=torch.nn.CrossEntropyLoss(), verbose=verbose, test_dataloader=test_dataloader)

        return acceptance_ratio

    async def train_1_batch(self, X, y, model, dataloader, loss_fn, verbose=False, test_dataloader=None):
        ...
        # écriture des logs ici !!!!! ça aura plus de sens
