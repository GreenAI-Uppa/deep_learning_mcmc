...
class Student(object):
    ...
    def __init__(self, s, m=0, df=1):
        ...
        self.gamma_factor = math.gamma(1. * (self.d+self.df)/2) / ( math.gamma(1.*self.df/2) * pow(self.df*math.pi,1.*self.d/2) * pow(np.linalg.det(self.S),1./2) ) # pourquoi multiplié par la variance ? 
    ...
    def sample(self, n=1):
        ...
        return (self.m + Z/np.sqrt(g)).reshape(n,) # renvoie un echantillon suivant une loi de student sur d dimension 
    ...
    def t_distribution_fast(self, x):
        ...
        '''
        assert self.df == 1
        Denom = 1 + x*x/self.S[0][0] # pourquoi divisé par la variance et non par df ?
