...
def parse_model_config(params, training_data):
    ...
    # building the model
    conv1 = nets.ConvNetAuxResult(params['architecture']['nb_filters'], channels, binary_flags=boolean_flags,  activations=activations)
    conv2 = nets.ConvNetAuxResult(params['architecture']['nb_filters'], params['architecture']['nb_filters'], binary_flags=boolean_flags,  activations=activations, kernel_size=3, stride=1, input_size=8)
    model = [conv1, conv2]
    return model
...
model = parse_model_config(params, training_data)
