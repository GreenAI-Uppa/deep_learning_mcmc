...
def parse_model_config(params, training_data):
    """ setting the model"""
    #return nets.ConvNet(64,3)
    return nets.SmallbaselineCifar()
...
for t in range(epochs):
    ...
    loss, accuracy = evaluator.evaluate(train_dataloader, model, loss_fn)
    print(f"Training Error: \n Accuracy: {(100*accuracy):>0.1f}%, Avg loss: {loss:>8f} \n")
    loss, accuracy = evaluator.evaluate(test_dataloader, model, loss_fn)
    print(f"Test Error: \n Accuracy: {(100*accuracy):>0.1f}%, Avg loss: {loss:>8f} \n")
